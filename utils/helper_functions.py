...
def encode_from_string(content, chunk_size=1000, chunk_overlap=200):
    """
    Encodes a string into a vector store using NVIDIA NIM embeddings.
...
async def retry_with_exponential_backoff(coroutine, max_retries=5):
    """
    Retries a coroutine using exponential backoff upon encountering rate limit errors.
...
def get_langchain_embedding_provider(provider: EmbeddingProvider = None, model_id: str = None):
    """
    Returns an NVIDIA NIM embedding provider.

    Args:
        provider (EmbeddingProvider): Ignored — always uses NVIDIA.
        model_id (str): Optional - The specific embeddings model ID to use.

    Returns:
        A LangChain NVIDIA embedding provider instance.
    """
    from langchain_nvidia_ai_endpoints import NVIDIAEmbeddings
    model = model_id or "nvidia/llama-nemotron-embed-1b-v2"
