...
# Try importing deepeval — optional, requires Python 3.10+
try:
...
def create_deep_eval_test_cases(
    ...
    Returns:
        List of LLMTestCase objects (requires deepeval + Python 3.10+).
    """
    if not DEEPEVAL_AVAILABLE:
        print("⚠️  deepeval not available (requires Python 3.10+). Skipping test case creation.")
        return []
...
def evaluate_rag(retriever, num_questions: int = 5) -> Dict[str, Any]:
    ...
    llm = ChatNVIDIA(temperature=0, model="meta/llama-3.3-70b-instruct")
